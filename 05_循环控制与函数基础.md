...
上一篇我们玩转了 while 循环，并使用 while 循环替代 for 循环计算 1 到 100 的整数相加之和，然后又分别使用了 while 循环和 for 循环来计算水仙花数。通过使用 while 无限循环和循环嵌套，对猜数字游戏进行了再一次的升级改造。

上一篇留了一道思考题：用 while 循环双重嵌套打印九九乘法表，有人做出来吗？

## break 和 continue

上一篇我们学习无限循环（也称死循环），关于死循环有人就说："死循环一旦跑起来，就再也没有回头路了……"。想一想真的只能这样吗？
...
随着学习Python的深入，编写的代码量将会不断增加，程序结构也日益复杂，那么我们需要找一个好的方法对这些复杂的代码进行梳理重构。回想一下上一篇最后讲到改进后的猜数字游戏中判断输入值有两段代码是完全相同的，本文档学习函数后就可以利用函数实现降低代码结构的复杂性和冗杂度，并且能够提升代码的可读性，下面简单总结了函数的作用和优点：
...
函数是Python编程中超级重要基础内容，Python 自身提供了许多内置函数，比如 print()、max()、min()、abs()、str()、list() 等等，内置函数非常的丰富，我们可以参考学习，同样也可以自定义实现其功能。要学好函数，应在文档中保持和老师同步，课后多练习。
...
学习了函数的参数和返回值，现在可以使用函数来优化上一篇的猜数字游戏了，先回顾一下上一篇最后猜数字游戏代码的样子。 上代码：
...
| breakpoint() | exec() | issubclass() | pow() | super() |
| bytearray() | filter() | iter() | print() | sum() |
...
| compile() | globals() | max() | round() | import() |
...
前面的文档我们学习了列表，元组和列表都属于序列，它们的关系就像表亲一样，有很多的相似点。
...
这篇文档我们学习了控制循环的两种方式：break 和 continue，它们的用法大家都搞清楚了吗？函数是我们迈向高阶编程的垫脚石，大家一定要通过多多编码练习来加强理解和运用。配合对元组打包和拆包的运用，在处理函数的传参和返回值时可以更加灵活。
...
# 输出：3 6 9 12 18 21 24 27 33 36 39 42 48 51 54 57 63 66 69 72 78 81 84 87 93 96 99
```
